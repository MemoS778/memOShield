...
var sampleCountries = []countryInfo{
...
func randomIP() string {
	if rand.Float64() < 0.3 && len(knownIPs) > 0 {
		return knownIPs[rand.Intn(len(knownIPs))]
	}
...
func main() {
	db.InitDB()
	n := 80
	fmt.Printf("Seeding %d mock events...\n", n)
	...
	for i := 0; i < n; i++ {
		...
		details := attackDetails[attack][rand.Intn(len(attackDetails[attack]))]

		_ = now.Add(-time.Duration(rand.Intn(1800)) * time.Second)

		db.LogEvent(
			ip, c.Name, attack, details,
			c.Lat+rand.Float64()-0.5,
			c.Lon+rand.Float64()-0.5,
		)
	}

	// Add some bans
	banReasons := []string{"Port Scan saldırısı", "DDoS tespit edildi", "Honeypot tetiklendi", "Brute force", "SQL Injection"}
	for i := 0; i < 5 && i < len(knownIPs); i++ {
		db.AddBan(knownIPs[i], banReasons[i])
	}

	fmt.Printf("Seeded %d events + 5 bans.\n", n)

	events := db.GetEvents(10)
	fmt.Println("\nLatest 10 events:")
	for _, e := range events {
		fmt.Printf("  %s  %-16s  %-12s  %s\n", e.Timestamp, e.SrcIP, e.Country, e.AttackType)
	}
}
